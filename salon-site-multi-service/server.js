...
app.post('/api/bookings', async (req, res) => {
  ...
  if (!name || !email || !Array.isArray(services) || services.length === 0 || !date || !time) {
  ...
  if (process.env.RESEND_API_KEY && fromEmail) {
    try {
      ...
      const servicesList = `<ul>${services.map(s => `<li>${s}</li>`).join('')}</ul>`;
      ...
      const html = `
        ...
        ${servicesList}
        <p><strong>Staff:</strong> ${staffName || 'Any available'}</p>
...
app.delete('/api/staff/:id', async (req, res) => {
  ...
  const remaining = shifts.filter(sh => sh.staffId !== id);
  await writeJson(SHIFTS_PATH, remaining);
